...
## Test that if you create a card with rank 1, its rank will be "Ace"
        ...
        ## Test that if you create a card instance, it will have access to a variablesuit_names that contains the list ["Diamonds","Clubs","Hearts","Spades"]
    def test_cardInstance(self):
        c = Card()
            self.assertEqual(c.suit_names,["Diamonds","Clubs","Hearts","Spades"])
    ## Test that if you invoke the __str__ method of a card instance thatis created with suit=2, rank=7, it returns the string "7 of Hearts"
        def test_cardInstance2(self):
            c = Card(suit=2, rank = 7)
                self.assertEqual(str(c),"7 of Hearts")
        ## Test that if you create a deck instance, it will have 52 cards inits cards instance variable
        def test_deckInstance(self):
            d = Deck()
                self.assertEqual(len(d.cards),52)
            ## Test that if you invoke the pop_card method on a deck, it willreturn a card instance.
